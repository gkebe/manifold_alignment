...
def train(experiment_name, epochs, train_data_path, dev_data_path, test_data_path, pos_neg_examples_file, batch_size, embedded_dim, gpu_num, seed, num_layers, h, awe, margin=0.4):
    ...
    epoch_files = sorted([int(f.split("_")[-1].split(".")[0]) for f in listdir(results_dir) if isfile(join(results_dir, f)) if ".txt" in f and "language2language_test_epoch" in f])
    current_epoch = epoch_files[-1]
    print(current_epoch)
    ...
    batch_loss = []
    avg_epoch_loss = [0]*current_epoch + []

    train_fout.write('epoch,step,target,pos,neg,case,pos_dist,neg_dist,loss\n')
    for epoch in tqdm(range(current_epoch, epochs), desc="Epoch"):
        epoch_loss = 0.0
