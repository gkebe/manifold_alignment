...
class LSTM(torch.nn.Module):
    def __init__(self, input_size, output_size, hidden_dim, num_layers, dropout, device, bilstm= False, more_fc=False):
        ...
        if bilstm:
            self.lstm = torch.nn.LSTM(input_size, hidden_dim/2, num_layers,
    ...
    def forward(self, X):
        ...
        out, (h_t, c_t) = self.lstm(X, (h_t, c_t))
        if self.bilstm:
            out = h_t.view(self.num_layers, batch_size, self.hidden_dim)[-1]
        else:
            out = h_t.view(self.num_layers, batch_size, self.hidden_dim)[-1]
