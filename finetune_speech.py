...
def train(experiment_name, epochs, train_data_path, pos_neg_examples_file, batch_size, gpu_num, lr=0.001, lstm=False):
    def lr_lambda(e):
        if e < 20:
            return lr
        elif e < 40:
    ...
    if lstm:
        ...
        speech_model = GRU(
            input_size=list(speech_train_data[0].size())[1],
            embedded_dim=list(speech_train_data[0].size())[1]
        )
    else:
#        speech_model = Combiner(list(speech_train_data[0].size())[1], embedded_dim)
        speech_model = SmarterAttentionNet(list(speech_train_data[0].size())[1], list(speech_train_data[0].size())[1])
