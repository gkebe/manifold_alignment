...
with open("wav2vec2_transcriptions.pkl",'rb') as f:
    w2v2_transcriptions = pickle.load(f, encoding='bytes')
...
for i in tqdm(range(0, len(dataset)), desc="Instance"):
    transcription = w2v2_transcriptions[dataset[i][0]][0]
    if transcription == "":
       transcription = "[UNK]"
    language_data.append(proc_sentence(transcription).detach().cpu())
...
with open('features/gld_w2v2transcriptions_vision_tensors.pkl', 'wb') as f:
    pickle.dump(data, f)
